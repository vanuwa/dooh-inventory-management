...
- An orange accent strip below the header names the active environment and its host whenever it is not production, so an acceptance session cannot be mistaken for a production one; switching from the header does a full reload to `/recent`, because cached page state carries IDs that mean nothing in the other instance, and other open tabs reload into the new selection too rather than keep showing the old one
