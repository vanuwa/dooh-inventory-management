...
up:
	GIT_COMMIT=$$(git rev-parse HEAD) GIT_BRANCH=$$(git rev-parse --abbrev-ref HEAD) docker compose up -d --build --force-recreate
...
rebuild-ui:
	...
	GIT_COMMIT=$$(git rev-parse HEAD) GIT_BRANCH=$$(git rev-parse --abbrev-ref HEAD) docker compose up -d --build ui
